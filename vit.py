...
class ViTClassfier(tf.keras.models.Model):
    ...
    def __init__(self, input_shape, n_classes, config={}):
        ...
        self.config = {**self.CONFIG, **config, "image_size": input_shape, "n_classes": n_classes}
    ...
    def _construct_model(self):
        config = self.config  # Use regular dictionary

        num_patches = (config['image_size'][0] // config['patch_size']) ** 2

        # Ensure image_size is passed as a tuple
        inputs = tf.keras.layers.Input(shape=(config['image_size'][0], config['image_size'][1], config['image_size'][2]))
